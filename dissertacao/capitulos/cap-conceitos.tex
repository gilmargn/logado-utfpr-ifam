...
A extensão LOGADO foi projetada para ser uma ferramenta leve e integrada ao fluxo natural do estudante. Após o desenvolvimento e testes, ela será disponibilizada no repositório oficial de extensões do VS Code, o \textbf{Visual Studio Code Marketplace} (Figura \ref{fig:marketplace}), de onde poderá ser facilmente instalada por qualquer usuário.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{marketplace-vscode.png} 
    \caption{Página inicial do Visual Studio Code Marketplace, ambiente onde a extensão LOGADO será publicada e disponibilizada para a comunidade.}
    \label{fig:marketplace}
\end{figure}

%\begin{itemize}
%\item Configuração Granular: O usuário poderá habilitar ou desabilitar a coleta de tipos específicos de eventos (ex., apenas execuções e erros, sem capturar cada edição de tecla).
%
%\item Gerenciamento de Privacidade: A extensão operará inicialmente em modo estritamente local. Os dados não serão transmitidos para servidores externos sem o consentimento explícito do usuário, assegurando a privacidade.
%
%\item Exportação de Dados: Será fornecida uma funcionalidade para exportar os logs em JSON para facilitar a análise por ferramentas externas ou para compartilhamento com instrutores, em cenários de pesquisa ou tutoria.
%
%\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Evidência do funcionamento do plugin LOGADO. Para cada arquivo JavaScript criado pelo estudante (ex., \texttt{aula.js}), a extensão gera automaticamente um arquivo de log homônimo no formato JSON (\texttt{aula.json}), conforme destacado. A captura mostra na aba do Explorador do VS Code os arquivos no diretório.}
