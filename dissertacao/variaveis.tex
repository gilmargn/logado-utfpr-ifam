...
\TipoDeDocumento{Proposta de Dissertação}%% Tipo de documento: "Tese", "Dissertação" ou "Trabalho de Conclusão de Curso de Graduação", "Estágio Supervisionado"
...
\program{Mestre em Computação Aplicada}
\programen{Pós-Graduação em Computação Aplicada}
\degree{Mestre}
...
% TCC2:
 \goal{Proposta de Dissertação}
 \course{Programa de Pós-Graduação em Computação aplicada}


% Dados do TCC (precisa alterar)
\author{GILMAR GOMES DO NASCIMENTO}  % Seu nome
\authorbib{Nascimento, Gilmar Gomes} % Seu nome para referência bibliográfica (Sobrenome, Nome)
\title{O USO DE LOGS NO ENSINO DE PROGRAMAÇÃO} % Título do trabalho

\titleFA{TÍTULO DO TRABALHO: SUBTÍTULO (SE HOUVER) PRECEDIDO DE DOIS PONTOS}

\titleen{USING LOGS TO SUPPORT PROGRAMMING EDUCATION} % Título traduzido para inglês
\advisor{Prof. Dr. Laudelino Cordeiro Bastos} % Nome do orientador. Lembre-se de prefixar com "Prof. Dr.", "Profª. Drª.", "Prof. Me." ou "Profª. Me."}
% Se não houver corientador, comente a linha a baixo
\coadvisor{Profa. Dra. Maria Claudia Figueireido Pereira Emer} % Nome do coorientador, caso exista. Caso não exista, comente a linha.
...
\universityunit{Programa de Pós-graduação de Computação Aplicada}
\address{Curitiba}
\addressen{Campus Curitiba, PR, Brazil}
\documenttype{Dissertação}
\documenttypeen{Dissertação}
\degreetype{Mestrado}
...
\PalavraChaveA{Ensino de Programa\c{c}\~{a}o}%% Palavra-chave A
\PalavraChaveB{Log}%% Palavra-chave B
\PalavraChaveC{Telemetria}%% Palavra-chave C
\PalavraChaveD{Logging}%% Palavra-chave D
...
\KeywordA{Programming Education}%% Keyword A
\KeywordB{log}%% Keyword B
\KeywordC{telemetry}%% Keyword C
\KeywordD{logging}%% Keyword D
