\chapter{Mapeamento Sistemático da Literatura}\label{chap:msl}

\section{Objetivo e questões de pesquisa}\label{sec:msl-objetivo}
...
\section{Estratégia de busca e repositórios}\label{sec:msl-estrategia}

\subsection{Repositórios consultados}\label{subsec:repositorios}
...
\subsection{Strings de busca}\label{subsec:strings-busca}
...
\subsection{Idioma dos trabalhos}\label{subsec:idioma-trabalhos}
...
\section{Critérios de inclusão e exclusão}\label{sec:msl-criterios}

\subsection{Critérios de inclusão}\label{subsec:criterios-inclusao}
...
\subsection{Critérios de exclusão}\label{subsec:criterios-exclusao}
...
\section{Resultados da busca e seleção de estudos}\label{sec:msl-resultados}
...
\section{Caracterização dos estudos incluídos}\label{sec:msl-caracterizacao}

A Tabela \ref{tab:caracteristicas} sumariza as principais características dos 17 estudos selecionados.
...
\begin{figure}[H]
    ...
    \label{fig:caracteristicas-distribuicao}
\end{figure}
...
\section{Respostas às questões de pesquisa}\label{sec:msl-respostas}

\subsection{QP1: Quais são as principais aplicações do uso de logs no ensino de programação?}\label{subsec:respostas-qp1}
...
\noindent \textbf{Em síntese}, as principais aplicações de logs no ensino de programação, identificadas nos 17 estudos analisados, concentram-se em quatro categorias: detecção de dificuldades (35\% dos estudos), análise de comportamento (29\%), qualidade de código (18\%) e práticas/ferramentas (18\%). Predominam abordagens voltadas à identificação precoce de estudantes com baixo desempenho e à análise de padrões de codificação.

\subsection{QP2: Quais ferramentas, métodos e métricas são mais utilizados para capturar, armazenar e analisar logs em ambientes de ensino de programação?}\label{subsec:respostas-qp2}
...
\noindent \textbf{Em síntese}, as ferramentas mais frequentes para captura de logs são IDEs específicas (CodeBench, Thonny), plugins para IDEs comerciais (Eclipse) e sistemas web (ProgEdu). Os métodos predominantes incluem registro de eventos de teclado, mouse, salvamento de arquivos e submissões. As métricas mais comuns são: número de compilações, taxa de erro por linha de código, tempo entre submissões, quantidade de tentativas até o acerto e padrões de espaçamento e indentação.

\section{Lacunas identificadas e trabalhos futuros}\label{sec:msl-lacunas}
